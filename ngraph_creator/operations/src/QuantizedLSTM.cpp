...
std::shared_ptr<ngraph::Node> QuantizedLSTM::createNode() {
    ...
    ALOGE("2 . In createNode after LN\n");
    ...
    ALOGE("3 . In createNode after LN\n");
    ...
    ALOGE("Before isLayerNormUsed *************** 23 \n");
    if (isLayerNormUsed) {
...
/*     auto outputIndex = sModelInfo->getOperationOutput(mNnapiOperationIndex, 1);
    mNgraphNodes->setOutputAtOperandIndex(outputIndex, LstmOutputs[1]);
    const auto op = sModelInfo->getOperand(outputIndex);
    if (op.lifetime == V1_3::OperandLifeTime::SUBGRAPH_OUTPUT) {
        addResultNode(outputIndex, LstmOutputs[1]);
    }

    outputIndex = sModelInfo->getOperationOutput(mNnapiOperationIndex, 2);
    mNgraphNodes->setOutputAtOperandIndex(outputIndex, LstmOutputs[2]);
    const auto op2 = sModelInfo->getOperand(outputIndex);
    addResultNode(outputIndex, LstmOutputs[2]); */
    ALOGE("In createNode after LN $$$$$$$$$$$$$$$$ Done Final 3\n");
    ...
    ALOGE("In createNode after LN $$$$$$$$$$$$$$$$ Done Final\n");
    return nullptr;
}
